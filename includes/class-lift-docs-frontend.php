...
class LIFT_Docs_Frontend {
    ...
    public function documents_shortcode($atts) {
        ...
        if (empty($documents)) {
            return '<p>' . __('No documents found.', 'lift-docs-system') . '</p>';
        }
        
        $output = '<div class="lift-docs-list">';
        
        foreach ($documents as $doc) {
            $output .= '<div class="lift-doc-item">';
            $output .= '<h3><a href="' . get_permalink($doc->ID) . '">' . esc_html($doc->post_title) . '</a></h3>';
            
            if ($atts['show_excerpt'] === 'true' && $doc->post_excerpt) {
                $output .= '<p class="excerpt">' . esc_html($doc->post_excerpt) . '</p>';
            }
            
            if ($atts['show_meta'] === 'true') {
                $output .= '<div class="doc-meta">';
                $output .= '<span class="date">' . get_the_date('', $doc->ID) . '</span>';
                
                $categories = get_the_terms($doc->ID, 'lift_doc_category');
                if ($categories && !is_wp_error($categories)) {
                    $output .= ' | <span class="category">' . $categories[0]->name . '</span>';
                }
